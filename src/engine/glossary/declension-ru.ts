 ...
 */
export function declineNameRu(firstName: string, gender: Gender, lastName?: string): Declensions {
  ...
  for (const case_ of cases) {
    try {
      ...
      result[case_] = lastName ? `${declined.first} ${declined.last}`.trim() : declined.first;
    } catch {
