    ...
    async (req, res) => {
      try {
        ...
        setImmediate(async () => {
          ...
          const currentJob = await deps.importJobStore.getJob(jobId);
          ...
          await deps.importJobStore.updateJob(jobId, {
            status: 'processing',
            phase: 'parsing',
          });
          ...
          try {
            ...
            if (extension === 'epub') {
              ...
              await deps.importJobStore.updateJob(jobId, { phase: 'saving' });
              req.log?.info(
                {
              ...
              for await (const parsedChapter of lazyResult.chapterIterator) {
                if (await deps.importJobStore.isCancelRequested(jobId)) {
                  await deps.importJobStore.updateJob(jobId, {
                    status: 'canceled',
                    finishedAt: new Date().toISOString(),
                  });
