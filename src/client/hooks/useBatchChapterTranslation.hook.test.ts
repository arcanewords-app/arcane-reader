...
describe('useBatchChapterTranslation', () => {
  afterEach(() => {
    vi.clearAllMocks();
  ...
  it('does not start when token check blocks without calling onProceed', () => {
    checkBeforeTranslate.mockImplementation(() => 'block');
    const onRefresh = vi.fn().mockResolvedValue(undefined);
    const { result } = renderHook(() => useBatchChapterTranslation('p1', makeProject(), onRefresh));
  ...
  it('cancel sets cancelled flag and clearProgress resets progress', async () => {
    translateChapter.mockImplementation(() => new Promise(() => {}));
    pollChapterUntilDone.mockImplementation(() => new Promise(() => {}));
    const onRefresh = vi.fn().mockResolvedValue(undefined);
