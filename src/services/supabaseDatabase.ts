...
// ============================================
// News posts & announcement alerts
// ============================================
...
interface AnnouncementAlertRow {
  id: string;
...
function transformNewsPostFromDB(row: NewsPostRow): NewsPost {
  return {
    id: row.id,
    slug: row.slug,
    title: row.title,
...
function transformAnnouncementFromDB(row: AnnouncementAlertRow): AnnouncementAlert {
  return {
    id: row.id,
...
function truncateAlertMessage(text: string, max = 160): string {
  const trimmed = text.trim();
  if (trimmed.length <= max) return trimmed;
  return trimmed.slice(0, max - 1).trimEnd() + '…';
}

function resolveAlertMessage(alert: AnnouncementAlertRow, newsSummary?: string | null): string {
  const raw = alert.message?.trim() || newsSummary?.trim() || '';
  return truncateAlertMessage(raw);
}

function isAnnouncementScheduledActive(row: AnnouncementAlertRow, now = new Date()): boolean {
...
function transformCatalogTranslationRequestInterestFromDB(
...
function toBoardTranslationRequest(
  row: CatalogTranslationRequestRow,
  interests: CatalogTranslationRequestInterest[],
...
function assertRequestOpenForBoard(request: CatalogTranslationRequest): void {
