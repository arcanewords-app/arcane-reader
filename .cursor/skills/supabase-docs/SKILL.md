...
**In this repo:** DB access is via `@src/services/supabaseDatabase.ts`; types in `@src/storage/types.ts`. Migrations: **only** `supabase/migrations/` (`@.cursor/rules/supabase.mdc`); history in `@supabase/README.md`. Auth middleware: `@src/middleware/auth.ts`. Profile `role` in `profiles` table — see `@.cursor/rules/auth.mdc`.
